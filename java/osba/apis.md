...
OSBA（Object-Oriented Scalable Business Architecture）模块是 Euonia 框架的核心业务对象系统，提供属性管理、规则检查、授权和属性变更通知等核心功能。该模块定义了业务对象的完整生命周期管理，包括创建、读取、更新、删除操作，以及基于注解的工厂模式和反射工具。
...
核心业务对象包，包含业务对象基类层次结构（BusinessObject → ObservableObject → EditableObject / ReadOnlyObject / ExecutableObject），以及业务上下文、可观察列表和事件参数类。
...
规则引擎包，提供业务对象验证规则的定义、执行和违规追踪，支持 Lambda 表达式规则、数据注解驱动规则、正则表达式规则和必填验证规则。
...
| [Rules](./com.euonia.osba.rules.Rules.md) | class | 规则执行器，负责管理业务对象实例的验证规则执行、跟踪已违反的规则并通知监听器 |
| [RuleManager](./com.euonia.osba.rules.RuleManager.md) | class | 按类型管理规则的线程安全容器，使用 ConcurrentHashMap 存储不同类型的规则列表 |
...
| [BrokenRule](./com.euonia.osba.rules.BrokenRule.md) | record | 已破坏的规则，包含属性名、违规描述和严重级别 |
| [BrokenRuleCollection](./com.euonia.osba.rules.BrokenRuleCollection.md) | class | 已破坏规则集合 |
| [RuleCheckException](./com.euonia.osba.rules.RuleCheckException.md) | class | 规则校验异常，携带按属性分组的校验错误，映射 HTTP 400 状态码 |
...
| [BusinessObjectFactory](./com.euonia.factory.BusinessObjectFactory.md) | class | ObjectFactory 接口的实现，通过反射发现带注解的工厂方法，支持与 ServiceProvider 集成 |
...
反射工具包，提供属性元数据管理（PropertyInfo）、字段数据追踪（FieldData）、工厂方法反射查找（ObjectReflector）以及相关异常类。
...
| [ObjectReflector](./com.euonia.reflection.ObjectReflector.md) | class | 反射工具类，提供基于注解的工厂方法查找，带缓存支持 |
