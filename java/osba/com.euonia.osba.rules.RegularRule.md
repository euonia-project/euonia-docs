...
> RegularRule 类实现了一个基于正则表达式的验证规则，用于验证业务对象的字符串属性是否符合指定的正则表达式模式。该规则允许配置是否忽略空值，并提供了一个消息工厂函数，用于根据属性值生成适当的错误消息。
...
- **Returns**: `RegularRule` — 当前 RegularRule 实例
...
> 创建一个新的 RegularRule 实例。

- **Parameters**:
  - `property` (`PropertyInfo<String>`): 属性信息
- **Returns**: `RegularRule` — 新的 RegularRule 实例
